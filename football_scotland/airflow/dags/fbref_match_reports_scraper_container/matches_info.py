...
from assign_change_team_name import assign_change_team_name
from extract_players_and_formation_from_lineup import extract_players_and_formation_from_lineup

def matches_info(all_match_reports_for_updating_param):
    match_reports_data_single_run = []
    for match_rpt_idx in range(len(all_match_reports_for_updating_param)):
            ...
            response_single_match = requests.get(all_match_reports_for_updating_param[match_rpt_idx])
            time.sleep(20)
            ...
            single_match_dict["match_report_url"] = all_match_reports_for_updating_param[match_rpt_idx]
        ...
        #    single_match_dict["game_venue_time"] =game_venue_time
            ...
            #team_stats_extra_div = soup_single_match.find('div', id='team_stats_extra')
            ...
            print(all_match_reports_for_updating_param[match_rpt_idx])
            #print(single_match_dict)
            #if single_match_dict in match_reports_single_season:
            #    continue
            #else:
            match_reports_data_single_run.append(single_match_dict)

 
    # Create a DataFrame
    untransformed_df = pd.DataFrame(match_reports_data_single_run)
    # Split the URL by '/'
    #parts_url_untransformed = standings_url_param.split('/')

    # Extract the relevant part (the one containing '2022-2023')
    #season_name_untransformed = parts_url_untransformed[6].replace("-","_") 
    #csv_name_untransformed = f"untransformed_df_{start_date}_{end_date}.csv"

    #untransformed_df.to_csv(csv_name_untransformed, index=False)
